...
if len(sys.argv[1:])==0:             #argv[1] will be the Search name to scrape data from
...
if save:
    ...
    try:
        os.mkdir(dir)       # Create a new directory/file for saving file
    except OSError:
        pass

    os.chdir(dir)           # Move the control to newly created directory
...
for i in range(0,5):     # Number of links to be displayed for scraping
    try:
        ...
        r = requests.get(temp_url,headers={'user-agent':'boom-boom'})      # Give any name for User agent(used for security)
        s = BeautifulSoup(r.text,'html.parser')
        p = s.select('p')
